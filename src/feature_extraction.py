...
def feature_extraction(df, tfidf_vectorizer=None, svd=None, fit=False, selected_feature_names=None):
    ...
    sentiment_scores = df[text_column].apply(lambda text: analyzer.polarity_scores(text)['compound']).values.reshape(-1,
                                                                                                                     1)
...
def load_data():
    ...
    if os.path.exists(train_features_path) and os.path.exists(tfidf_vectorizer_path) and os.path.exists(
            svd_path):  # and os.path.exists(feature_names_path):
    ...
    if os.path.exists(val_features_path):
        ...
        X_val_combined, _, _, _ = feature_extraction(X_val, tfidf_vectorizer, svd, fit=False,
    ...
    if os.path.exists(test_features_path):
        ...
        X_test_combined, _, _, _ = feature_extraction(X_test, tfidf_vectorizer, svd, fit=False,
                                                      selected_feature_names=selected_feature_names)
...
def explain_and_plot_top_features(
    ...
    if model_type == "lgb":
        ...
        feature_names = X_test_features.columns if hasattr(X_test_features, "columns") else [f"f{i}" for i in range(
    ...
    if model_type == "lgb":
        ...
        if len(model.feature_importances_) == X_test_features.shape[1]:
            shap_values = []
            for start in tqdm(range(0, X_test_features.shape[0], batch_size), desc="Computing SHAP values",
                              unit="batch"):
            ...
            shap_values = []
            for start in tqdm(range(0, X_test_features[top_columns].shape[0], batch_size), desc="Computing SHAP values",
                              unit="batch"):
